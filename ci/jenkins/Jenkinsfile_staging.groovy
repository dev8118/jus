def config = [
  ...
  image: 'registry.saritasa.com/jlp/frontend:staging'
]
    ...
    stage('deploy') {
      gitlabCommitStatus('deploy') {
        withKubeConfig(credentialsId: 'saritasa-k8s-develop-token', serverUrl: "${K8S_DEVELOPMENT_URL}") {
          sh("helm --namespace ${config.namespace} upgrade --cleanup-on-fail --install ${config.project} ci/chart/staging/")
        }
      }
    }
  } catch (error) {
